...
$columns = array(
	...
	array( 'db' => '`ua`.`table`', 'dt' => 'table', 'field' => 'table' ),
...
$joinQuery = "FROM `tbl_invoice` AS `u` LEFT JOIN `tbl_res_reservation_table` AS `ua` ON(`u`.`idtbl_invoice` = `ua`.`idtbl_res_reservation_table`)";

// $joinQuery = "FROM `tbl_invoice` AS `u`";
 
if(!empty($_POST['search_date'])){ 
$date = $_POST['search_date'];
$extraWhere = "`u`.`status` IN (1,2) AND `u`.invdate = '$date'";
...
$extraWhere = "`u`.`status` IN (0,1) AND `u`.invdate BETWEEN '$startDate' AND '$endDate'";

}elseif(!empty($_POST['search_month'])){
$month = $_POST['search_month'];
$month_arr = explode('-',$month);
$extraWhere = "`u`.`status` IN (0,1) AND YEAR(`u`.invdate) = '$month_arr[0]' AND Month(`u`.invdate) = '$month_arr[1]'";
...
$extraWhere = "`u`.`status` IN (1,2) AND `u`.invdate BETWEEN '$from_date' AND '$to_date'";
