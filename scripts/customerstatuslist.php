...
$columns = array(
	...
	array( 'db' => '`ua`.`idtbl_res_kot`', 'dt' => 'idtbl_res_kot', 'field' => 'idtbl_res_kot' ),
...
$joinQuery = "FROM `tbl_res_kot_detail` AS `u` 
LEFT JOIN `tbl_res_kot` AS `ua` ON (`ua`.`idtbl_res_kot` = `u`.`tbl_res_kot_idtbl_res_kot`) 
LEFT JOIN `tbl_res_item` AS `ub` ON (`ub`.`idtbl_res_item` = `u`.`tbl_res_item_idtbl_res_item`) 
LEFT JOIN `tbl_res_reservation_table` AS `uc` ON (`uc`.`idtbl_res_reservation_table` = `ua`.`tableid`)";
$extraWhere = "`u`.`status` IN (1, 2) AND `ua`.`orderid` = 0 AND `ua`.`tableid` = 0";
