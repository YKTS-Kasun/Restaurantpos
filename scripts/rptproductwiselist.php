...
$columns = array(
    ...
    array( 'db' => '`ub`.`idtbl_res_item`', 'dt' => 'idtbl_res_item', 'field' => 'idtbl_res_item' ),
...
$joinQuery = "FROM `tbl_invoice` AS `u` LEFT JOIN `tbl_invoice_detail` AS `ua` ON (`u`.`idtbl_invoice` = `ua`.`tbl_invoice_idtbl_invoice`)
...
$itemname = $_POST['item'];
$extraWhere = "`u`.`status` IN (1,2) AND `u`.invdate = '$date' AND `ub`.idtbl_res_item = '$itemname'";
...
$extraWhere = "`u`.`status` IN (0,1) AND `u`.invdate BETWEEN '$startDate' AND '$endDate' AND `ub`.idtbl_res_item = '$itemname'";
...
$extraWhere = "`u`.`status` IN (0,1) AND YEAR(`u`.invdate) = '$month_arr[0]' AND Month(`u`.invdate) = '$month_arr[1]' AND `ub`.idtbl_res_item = '$itemname'";
...
$extraWhere = "`u`.`status` IN (1,2) AND `u`.invdate BETWEEN '$from_date' AND '$to_date' AND `ub`.idtbl_res_item = '$itemname'";
