				...
				<div class="card">
					<div class="card-body">
						<div class="row">
							...
							<div id="maindiv" class="col-sm-12 col-md-12 col-lg-2 col-xl-2" style="border-right: 1px dotted #000;">
								<div class="accordion" id="accordionExample">
									<div class="card shadow-none border-0">
										<div id="collapseOne" class="collapse show" aria-labelledby="headingOne" data-parent="#accordionExample">
											<h6 class="title-style mb-3"><span>Category</span></h6>
											<div class="card-body border rounded" style="height: 550px; overflow-y:auto; background-color:#FAF7DB;" id="style-2">
												<div class="row row-cols-1 row-cols-md-1">
													<?php foreach ($category->result() as $rowcategory) { ?>
													<div class="col mb-4 categorydiv" tabindex="0" id="<?php echo $rowcategory->idtbl_res_item_category ?>">
														<div class="card card-material h-100 shadow-none bg-warning border-warning">
															<div class="card-body p-2 text-center pointer">
																<h4 class="text-light font-weight-bold" style="font-size:20px;"><?php echo $rowcategory->categoryname ?></h4>
															</div>
														</div>
													</div>
													<?php } ?>
							...
							<div id="maindiv" class="col-sm-12 col-md-12 col-lg-5 col-xl-5" style="border-right: 1px dotted #000;">
								<div class="accordion" id="accordionExample">
									<div class="card shadow-none border-0">
										<div id="collapseOne" class="collapse show" aria-labelledby="headingOne" data-parent="#accordionExample">
											...
											<div class="card-body border rounded mb-2" id="style-2">
												<div class="row">
													...
													<div class="col-3">
														<button type="button" id="tablepaymentbtn" class="btn btn-secondary btn-sm mr-2 w-100" data-toggle="modal" data-target="#modaltablepayment"
															<?php if($addcheck==0){echo 'disabled';} ?>>
											...
											<div class="card-body border rounded">
												<div class="row">
													...
													<div class="col-12" id="divproductlist">
														<table class="table table-bordered table-striped table-sm" id="tableproductpricelist">
															<thead>
																<tr>
																	<th>#</th>
...
<!-- Modal Table -->
<div class="modal fade" id="modaltable" data-backdrop="static" data-keyboard="false" tabindex="-1"
	aria-labelledby="staticBackdropLabel" aria-hidden="true">
	...
	<div class="modal-dialog modal-dialog-centered modal-sm">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="row">
				<div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
						<button id="btnprintinvoice" type="button" class="btn btn-dark btn-sm w-100 mb-3"><i
								class="fas fa-print fa-3x mr-2"></i>
							<h1 class="font-weight-normal mt-2 text-light">PRINT</h1>
						</button>
					</div>
					<div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
...
<!-- Modal HTML -->
<div class="modal fade" id="paymentModal" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog"
			...
			<div class="modal-body">
				<form id="paymentForm">
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" id="modalDiscountPercent"
...
<div class="modal fade" id="discountApprovalModal" data-backdrop="static" data-keyboard="false" tabindex="-1"
	role="dialog" aria-labelledby="discountApprovalModalLabel" aria-hidden="true">
	<div class="modal-dialog" role="document">
...
<!-- Payment Modal -->
<div class="modal fade" id="paymentaddModal" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog"
	aria-labelledby="paymentModalAddLabel" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
			...
			<div class="modal-body">
				<form id="paymentaddForm">
					...
					<div class="form-group">
						<label for="grossTotal">Net Total</label>
					...
					<div class="form-group" id="last4DigitsField" style="display:none;">
						<label for="last4Digits">Last 4 Digits of Card</label>
						<input type="text" class="form-control" id="last4Digits" maxlength="4" placeholder="Enter last 4 digits">
					</div>
					<div class="form-group">
...
<!-- Discount Approval Modal -->
<div class="modal fade" id="tablediscountApprovalModal" data-backdrop="static" data-keyboard="false" tabindex="-1"
	role="dialog" aria-labelledby="tablediscountApprovalModalLabel" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content bg-danger-soft">
			<div class="modal-header">
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		$('#saveAddPayment').on('click', function () {
			...
			if (tablediscountPercent !== '' && tablediscountPercent > 0 && tableapprovestatus === '' &&
				tableapproveuser === '') {
	...
	$(document).ready(function () {
		$(document).on('click', '.btnaddpayments', function () {
			...
			var nettotal = parseFloat($(this).data('nettotal'));

			$('#invoiceID').val(invoiceID);
			$('#grossTotal').val(nettotal.toFixed(2));
		...
		$("#paymentmode").change(function() {
			if ($(this).val() == "2") {
				$("#last4DigitsField").show();
			} else {
				$("#last4DigitsField").hide();
				$("#last4Digits").val(""); 
			}
		});
	...
	$(document).ready(function () {
		...
		$(document).on("click", ".btnaddtocart", function () {
			...
			$('#carttable > tbody:last').append('<tr class="pointer ' + classname + '"><td>' + product +
				'</td><td class="text-center">' + qty + '</td><td class="text-right">' + parseFloat(
					finalsaleamount).toFixed(2) +
				'</td><td class="text-right"></td><td class="text-right">' + showtotal +
				'</td><td class="d-none">' + productID + '</td><td class="d-none">' + finalsaleamount +
				'</td><td class="total d-none">' + total + '</td><td class="d-none">' + discount +
				'</td><td class="distotal d-none">' + discount + '</td><td class="nettotal d-none">' +
				totalwithdis + '</td><td class="d-none">' + editstatus + '</td></tr>');
		...
		$("#btnaddtolist").click(function () {
			if (!$("#formqtyadd")[0].checkValidity()) {
				...
				$('#carttable > tbody:last').append('<tr class="pointer ' + classname + '"><td>' +
					productcode + '</td><td class="text-center">' + qty +
					'</td><td class="text-right">' + parseFloat(finalsaleamount).toFixed(2) +
					'</td><td class="text-right">' + parseFloat(discountamount).toFixed(2) +
					'</td><td class="text-right">' + showtotal + '</td><td class="d-none">' +
					productID + '</td><td class="d-none">' + productcode + '</td><td class="d-none">' +
					finalsaleamount + '</td><td class="d-none">' + unit +
					'</td><td class="total d-none">' + total + '</td><td class="d-none">' +
					discountpresentage + '</td><td class="distotal d-none">' + discountamount +
					'</td><td class="nettotal d-none">' + totalwithdis + '</td><td class="d-none">' +
					editstatus + '</td></tr>');
				...
				var netsumnotser = 0;
				$(".nettotal").each(function () {
					netsumnotser += parseFloat($(this).text());
				});

				var servicecharge = 0;
				if (tableID != '') {
					var servicecharge = parseFloat((netsumnotser * 10) / 100);
				}

				var netsum = parseFloat(netsumnotser) + parseFloat(servicecharge);

				var showservicecharge = parseFloat(servicecharge).toFixed(2);
				var shownetsum = parseFloat(netsum + servicecharge).toFixed(2);

				$('#labeltotal').html('Gross Amount: ' + showsum);
				$('#labeldistotal').html('Discount: ' + showdissum);
				$('#labelservice').html('Service charge: ' + showservicecharge);
				$('#labelnettotal').html(shownetsum);
				$('#htmlbillamount').html('Rs. ' + shownetsum);
				$('#hiddenfulltotal').val(sum);
				$('#hiddenfulldistotal').val(dissum);
			...
			$.ajax({
				...
				success: function (response) {
					...
					if (response.length > 0) {
						...
						$('#btnpro').append(
							'<p class="text-dark">No tables available for this category.</p>'
							);
					}
		...
		$(document).on('click', '.tablediv', function () {
			...
			$.ajax({
				...
				success: function (result) {
					...
					// Net sum without service charge
					var netsumnotser = parseFloat(total_sum).toFixed(2);

					// Calculate the service charge
					var servicecharge = 0;
					if (table) {
						servicecharge = parseFloat((netsumnotser * 10) / 100);
					}
					var showservicecharge = parseFloat(servicecharge).toFixed(2);

					// Net sum including service charge
					var netsum = parseFloat(netsumnotser) + servicecharge;
					var shownetsum = parseFloat(netsum).toFixed(2);
					...
					$('#hiddenfullservice').val(showservicecharge);
					$('#hiddenfullnettotal').val(netsum);
					$('#amount').val(netsum);
		...
		$('#paymentbtn').click(function () {
			...
			if (tableID) {
				...
				$('#btncash, #btncredit, #btnpickme, #btnuber, #btnfree').hide();
			} else {
				$('#btnprintinvoice').hide();
				$('#btncash, #btncredit, #btnpickme, #btnuber, #btnfree').show();
			}
		});
		...
		$('#btnpickme').click(function () {
			...
			$('#tablepayment > tbody:last').append(
				'<tr class="pointer"><td class="d-none">1</td><td>Cash</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td class="d-none paytotal">' +
				fulltotal + '</td><td class="text-right">' + parseFloat(fulltotal).toFixed(2) +
				'</td></tr>');
			$('#billtype').val('4');
			createinvoice();
		});
		...
		$('#btnpayaddlist').click(function () {
			if (!$("#paymentform")[0].checkValidity()) {
				...
				$('#tablepayment > tbody:last').append('<tr class="pointer"><td class="d-none">' +
					paymentmethod + '</td><td>' + paymethod + '</td><td>' + bank + '</td><td>' +
					branch + '</td><td>' + chequeno + '</td><td>' + chequedate +
					'</td><td class="d-none paytotal">' + amount + '</td><td class="text-right">' +
					addCommas(parseFloat(amount).toFixed(2)) + '</td></tr>');
				...
				$("input[type=radio][name='paymentmethod']").prop('checked', false).parent().removeClass(
					'active');
				$('#bank').prop('readonly', true).prop('required', false);
				$('#branch').prop('readonly', true).prop('required', false);
				$('#chequeno').prop('readonly', true).prop('required', false);
	...
	function createinvoice() {
		...
		if (tbodysecond.children().length > 0 && billtype == 1) {
			...
			var paystatus = '1';
		} else if (tbodysecond.children().length > 0 && billtype == 4) {
			var paystatus = '1';
		} else if (tbodysecond.children().length > 0 && billtype == 5) {
			var paystatus = '1';
		} else if (tbodysecond.children().length > 0 && billtype == 6) {
			var paystatus = '1';
		} else {
			var paystatus = '0';
		}
	...
	function createtempinvoice() {
		...
		var distotal = $('#hiddenfulldistotal').val();
		var nettotal = $('#hiddenfullnettotal').val();
		var servicecharge = $('#hiddenfullservice').val();
		var paytotal = $('#hidepaymenttotal').val();
		var billtype = $("#billtype").val();
		var cusname = $('#cusname').val();
		var cusnic = $('#cusnic').val();
	...
	function addpayments() {
		...
		var last4Digits = (paymentmode == "2") ? $("#last4Digits").val() : null;
