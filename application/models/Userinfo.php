...
class Userinfo extends CI_Model{
    ...
    public function Usertypeedit(){
        ...
        $this->db->select('*');
        $this->db->from('tbl_res_user_type');
        $this->db->where('idtbl_res_user_type', $recordID);
        $this->db->where('status', 1);

        $respond=$this->db->get();

        $obj=new stdClass();
        $obj->id=$respond->row(0)->idtbl_res_user_type;
    ...
    public function Usertypeinsertupdate(){
        ...
        if($recordOption==1){
            ...
            $this->db->insert('tbl_res_user_type', $data);
            ...
            $this->db->where('idtbl_res_user_type', $recordID);
            $this->db->update('tbl_res_user_type', $data);
    ...
    public function Menulist(){
        $this->db->select('idtbl_res_menu_list, menu');
        $this->db->from('tbl_res_menu_list');
        $this->db->where('status', 1);
        ...
        if($recordOption==1){
            foreach($menulist as $rowmenulist){
                $data = array(
                    ...
                    'status'=>'1',
                    'insertdatetime'=>$updatedatetime,
                    'tbl_res_user_idtbl_res_user'=>$userlist,
                    'tbl_res_menu_list_idtbl_res_menu_list'=>$rowmenulist
                );

                $this->db->insert('tbl_res_privilege', $data);
            }
            ...
            foreach($menulist as $rowmenulist){
                ...
                $this->db->where('idtbl_res_privilege', $recordID);
                $this->db->update('tbl_res_privilege', $data);
            }
    ...
    public function Userprivilegeedit(){
        ...
        $this->db->select('*');
        $this->db->from('tbl_res_privilege');
        $this->db->where('idtbl_res_privilege', $recordID);
        $this->db->where('status', 1);
        ...
        $objmenulist->menulistID=$respond->row(0)->tbl_res_menu_list_idtbl_res_menu_list;
        array_push($menulistArray, $objmenulist);

        $obj=new stdClass();
        $obj->id=$respond->row(0)->idtbl_res_privilege;
        ...
        $obj->user=$respond->row(0)->tbl_res_user_idtbl_res_user;
